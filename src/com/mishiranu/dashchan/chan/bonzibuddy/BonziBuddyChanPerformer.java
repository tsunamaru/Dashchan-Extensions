...
public class BonziBuddyChanPerformer extends ChanPerformer {
	@Override
	public ReadThreadsResult onReadThreads(ReadThreadsData data) throws HttpException, InvalidResponseException {
		BonziBuddyChanLocator locator = BonziBuddyChanLocator.get(this);
		...
		if (jsonObject != null && data.pageNumber >= 0) {
			try {
				...
				for (int i = 0; i < threads.length; i++) {
					threads[i] = BonziBuddyModelMapper.createThread(threadsArray.getJSONObject(i),
				...
				try {
					...
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						for (int j = 0; j < threadsArray.length(); j++) {
							threads.add(BonziBuddyModelMapper.createThread(threadsArray.getJSONObject(j),
			...
			try {
				...
				if (jsonArray.length() > 0) {
					...
					for (int i = 0; i < posts.length; i++) {
						posts[i] = BonziBuddyModelMapper.createPost(jsonArray.getJSONObject(i),
	...
	public ReadBoardsResult onReadBoards(ReadBoardsData data) throws HttpException, InvalidResponseException {
		BonziBuddyChanLocator locator = BonziBuddyChanLocator.get(this);
		Uri uri = locator.buildPath("sp", "");
		String responseText = new HttpRequest(uri, data.holder, data).read().getString();
		try {
			return new ReadBoardsResult(new BonziBuddyBoardsParser(responseText).convert());
	...
	public SendPostResult onSendPost(SendPostData data) throws HttpException, ApiException, InvalidResponseException {
		...
		if (errorMessage != null) {
			...
			CommonUtils.writeLog("76chan send message", errorMessage);
	...
	public SendDeletePostsResult onSendDeletePosts(SendDeletePostsData data) throws HttpException, ApiException,
		...
		if (errorMessage != null) {
			...
			CommonUtils.writeLog("76chan delete message", errorMessage);
			throw new ApiException(errorMessage);
		}
		throw new InvalidResponseException();
	}
	...
	public SendReportPostsResult onSendReportPosts(SendReportPostsData data) throws HttpException, ApiException,
		...
		String errorMessage = jsonObject.optString("error");
		if (errorMessage != null) {
			CommonUtils.writeLog("76chan report message", errorMessage);
			throw new ApiException(errorMessage);
		}
		throw new InvalidResponseException();
	}
