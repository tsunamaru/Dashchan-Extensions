...
public class TumbachChanPerformer extends ChanPerformer {
	@Override
	public ReadThreadsResult onReadThreads(ReadThreadsData data) throws HttpException, InvalidResponseException {
		...
		JSONObject jsonObject = new HttpRequest(uri, data.holder, data).setValidator(data.validator).read()
				.getJsonObject();
		if (jsonObject != null) {
			TumbachChanConfiguration chanConfiguration = TumbachChanConfiguration.get(this);
			int pagesCount = jsonObject.optInt("pageCount");
			if (pagesCount > 0) {
				chanConfiguration.storePagesCount(data.boardName, pagesCount);
			}
			...
			try {
				ReadThreadsResult result = new ReadThreadsResult(TumbachModelMapper
						.createThreads(jsonObject.getJSONArray("threads"), locator, data.boardName));
				if (boardSpeedInt > 0) {
					result.setBoardSpeed(boardSpeedInt);
				}
				return result;
	...
	public ReadPostsResult onReadPosts(ReadPostsData data) throws HttpException, InvalidResponseException {
		...
		if (data.partialThreadLoading && data.lastPostNumber != null) {
			...
			if (jsonObject != null) {
				...
				if (lastPostNumber <= clientLastPostNumber) {
					return null;
				}
			} else {
				throw new InvalidResponseException();
		...
		JSONObject jsonObject = new HttpRequest(uri, data.holder, data).setValidator(data.validator)
				.read().getJsonObject();
		try {
			return new ReadPostsResult(TumbachModelMapper.createPosts(jsonObject.getJSONObject("thread"),
	...
	public ReadSinglePostResult onReadSinglePost(ReadSinglePostData data) throws HttpException,
		...
		if (jsonObject != null) {
			try {
				return new ReadSinglePostResult(TumbachModelMapper.createPost(jsonObject, locator, data.boardName));
			} catch (JSONException e) {
				throw new InvalidResponseException(e);
			}
		}
		if ("null".equals(response.getString())) {
			throw HttpException.createNotFoundException();
		}
		throw new InvalidResponseException();
	}
	...
	public ReadBoardsResult onReadBoards(ReadBoardsData data) throws HttpException, InvalidResponseException {
		...
		if (jsonObject != null) {
			try {
				...
				TumbachChanConfiguration configuration = TumbachChanConfiguration.get(this);
		...
		if (jsonObject != null) {
			...
			if (qouta > 0) {
				return new ReadCaptchaResult(CaptchaState.SKIP, null)
						.setValidity(TumbachChanConfiguration.Captcha.Validity.IN_BOARD);
			}
		}
		String captchaType = data.captchaType;
		boolean nodeCaptcha = TumbachChanConfiguration.CAPTCHA_TYPE_NODE_CAPTCHA.equals(captchaType);
		boolean recaptcha2 = TumbachChanConfiguration.CAPTCHA_TYPE_RECAPTCHA_2.equals(captchaType);
		boolean recaptcha1 = TumbachChanConfiguration.CAPTCHA_TYPE_RECAPTCHA_1.equals(captchaType);
	...
	public SendPostResult onSendPost(SendPostData data) throws HttpException, ApiException, InvalidResponseException {
		...
		if (data.captchaData != null) {
			if (TumbachChanConfiguration.CAPTCHA_TYPE_NODE_CAPTCHA.equals(data.captchaType)) {
			...
			} else if (TumbachChanConfiguration.CAPTCHA_TYPE_RECAPTCHA_2.equals(data.captchaType)) {
				entity.add("captchaEngine", "google-recaptcha");
				entity.add("g-recaptcha-response", data.captchaData.get(CaptchaData.INPUT));
			} else if (TumbachChanConfiguration.CAPTCHA_TYPE_RECAPTCHA_1.equals(data.captchaType)) {
		...
		if (!StringUtils.isEmpty(errorDescription)) {
			...
			CommonUtils.writeLog("Tumbach post message", errorDescription);
	...
	public SendDeletePostsResult onSendDeletePosts(SendDeletePostsData data) throws HttpException, ApiException,
		...
		if (data.optionFilesOnly) {
			...
			if (wasDeleted) {
				return null; // Ignore message if at least 1 image was deleted
			}
		} else {
		...
		CommonUtils.writeLog("Tumbach delete message", errorDescription);
		throw new ApiException(errorDescription);
	}
}