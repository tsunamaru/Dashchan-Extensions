...
public class NulleuChanPerformer extends ChanPerformer
{
	...
	public ReadThreadsResult onReadThreads(ReadThreadsData data) throws HttpException, InvalidResponseException
	{
		...
		if (data.isCatalog())
		{
			...
			try
			{
				posts = new NulleuPostsParser(responseText, this, data.boardName).convertPosts();
		...
		else
		{
			...
			try
			{
				return new ReadThreadsResult(new NulleuPostsParser(responseText, this, data.boardName)
						.convertThreads());
			}
			catch (ParseException e)
			{
			...
			InvalidResponseException
	{
		...
		if (lastPostNumber != null)
		{
			...
			if (!StringUtils.isEmpty(responseText))
			{
				try
				{
					posts = new NulleuPostsParser(responseText, this, data.boardName, data.threadNumber)
							.convertPosts();
				}
				catch (ParseException e)
				{
			...
			if (posts == null || posts.isEmpty())
			{
				...
				responseText = new HttpRequest(uri, data.holder, data).read().getString();
				if (!responseText.contains("<div id=\"thread")) throw HttpException.createNotFoundException();
			}
		...
		else
		{
			...
			try
			{
				return new ReadPostsResult(new NulleuPostsParser(responseText, this, data.boardName).convertPosts());
			}
			catch (ParseException e)
			{
				throw new InvalidResponseException(e);
	...
	public ReadSinglePostResult onReadSinglePost(ReadSinglePostData data) throws HttpException, InvalidResponseException
	{
		...
		try
		{
			Post post = new NulleuPostsParser(responseText, this, data.boardName).convertSinglePost();
	...
	public ReadBoardsResult onReadBoards(ReadBoardsData data) throws HttpException, InvalidResponseException
	{
		...
		try
		{
			return new ReadBoardsResult(new NulleuBoardsParser(responseText).convert());
		}
		catch (ParseException e)
		{
			throw new InvalidResponseException(e);
	...
	public SendPostResult onSendPost(SendPostData data) throws HttpException, ApiException, InvalidResponseException
	{
		...
		if (matcher.find())
		{
			...
			else if (errorMessage.contains("Вы забанены"))
			{
				...
				while (matcher.find())
				{
					...
					if (parseSuccess || startDateFound)
					{
						if (startDateFound)
						{
							if (parseSuccess) banExtra.setExpireDate(date);
							else if (group.contains("не истечет")) banExtra.setExpireDate(Long.MAX_VALUE);
							extra = banExtra;
							break;
						}
						else
					...
					else banExtra.setMessage(group);
				}
			}
			else if (errorMessage.contains("Blacklisted link"))
			{
				...
				if (matcher.matches()) extra = new ApiException.WordsExtra().addWord(matcher.group(1));
			}
			if (errorType != 0) throw new ApiException(errorType, extra);
			CommonUtils.writeLog("Nulleu send message", errorMessage);
			...
			InvalidResponseException
	{
		...
		if (responseText != null)
		{
			...
			CommonUtils.writeLog("Nulleu delete message", responseText);
		}
		throw new InvalidResponseException();
	}
}