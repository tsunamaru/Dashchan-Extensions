...
public class NulldvachnetChanPerformer extends ChanPerformer
{
	@Override
	public ReadThreadsResult onReadThreads(ReadThreadsData data) throws HttpException, InvalidResponseException
	{
		NulldvachnetChanLocator locator = NulldvachnetChanLocator.get(this);
		...
		try
		{
			return new ReadThreadsResult(new NulldvachnetPostsParser(responseText, this, data.boardName)
					.convertThreads());
		}
	...
	public ReadPostsResult onReadPosts(ReadPostsData data) throws HttpException, InvalidResponseException
	{
		...
		try
		{
			return new ReadPostsResult(new NulldvachnetPostsParser(responseText, this, data.boardName).convertPosts());
		}
		catch (ParseException e)
		{
			throw new InvalidResponseException(e);
	...
	{
		NulldvachnetChanLocator locator = NulldvachnetChanLocator.get(this);
		Uri uri = locator.buildPath("HOME", "menu.html");
		String responseText = new HttpRequest(uri, data).read().getString();
		try
		{
			return new ReadBoardsResult(new NulldvachnetBoardsParser(responseText).convert());
		}
		catch (ParseException e)
		{
			throw new InvalidResponseException(e);
	...
	public ReadCaptchaResult onReadCaptcha(ReadCaptchaData data) throws HttpException, InvalidResponseException
	{
		...
		if (image != null)
		{
			...
			canvas.drawBitmap(image, 0f, (newImage.getHeight() - image.getHeight()) / 2, null);
			image.recycle();
	...
	public SendPostResult onSendPost(SendPostData data) throws HttpException, ApiException, InvalidResponseException
	{
		...
		if (data.attachments != null)
		{
			...
			if (attachment.optionSpoiler) entity.add("img_spoiler", "on");
		}
		...
		if (matcher.find())
		{
			...
			if (errorMessage != null)
			{
				int errorType = 0;
				int flags = 0;
				if (errorMessage.contains("Плохая, негодная капча") || errorMessage.contains("неверная капча"))
				{
					errorType = ApiException.SEND_ERROR_CAPTCHA;
					flags |= ApiException.FLAG_KEEP_CAPTCHA;
				}
				else if (errorMessage.contains("не введен текст поста"))
				{
					errorType = ApiException.SEND_ERROR_EMPTY_COMMENT;
					flags |= ApiException.FLAG_KEEP_CAPTCHA;
				}
				else if (errorMessage.contains("постинг запрещен"))
				{
					errorType = ApiException.SEND_ERROR_EMPTY_FILE;
					flags |= ApiException.FLAG_KEEP_CAPTCHA;
				}
				else if (errorMessage.contains("тред несуществует"))
				{
			...
			CommonUtils.writeLog("Nulldvachnet send message", errorMessage);
			...
			InvalidResponseException
	{
		...
		if (matcher.find())
		{
			...
			{
				int errorType = 0;
				if (errorMessage.contains("неверный пароль на удаление"))
				{
					errorType = ApiException.DELETE_ERROR_PASSWORD;
				}
				if (errorType != 0) throw new ApiException(errorType);
			}
			CommonUtils.writeLog("Nulldvachnet delete message", errorMessage);
			throw new ApiException(errorMessage);
		}
		throw new InvalidResponseException();
	}
