...
{
	public static FileAttachment createFileAttachment(JSONObject jsonObject, ChanLocator locator)
			throws JSONException
	{
		...
		if ("/spoiler.png".equals(thumb)) attachment.setSpoiler(true);
		else if (!StringUtils.isEmpty(thumb)) attachment.setThumbnailUri(locator, locator.buildPath(thumb));
		if (!StringUtils.isEmpty(originalName)) attachment.setOriginalName(StringUtils.clearHtml(originalName));
		return attachment;
	}
	...
	public static Post createPost(JSONObject jsonObject, ChanLocator locator, String threadNumber)
			throws JSONException, ParseException
	{
		Post post = new Post();
		if (jsonObject.optInt("pinned") != 0) post.setSticky(true);
		if (jsonObject.optInt("locked") != 0) post.setClosed(true);
		if (jsonObject.optInt("cyclic") != 0) post.setCyclical(true);
		if (threadNumber != null)
		{
			post.setParentPostNumber(threadNumber);
			post.setPostNumber(CommonUtils.getJsonString(jsonObject, "postId"));
		}
		else post.setPostNumber(CommonUtils.getJsonString(jsonObject, "threadId"));
		...
		if (flag != null)
		{
			...
			else flagName = StringUtils.clearHtml(flagName);
	...
	public static Posts createPosts(JSONObject jsonObject, ChanLocator locator) throws JSONException, ParseException
	{
		...
		else
		{
			...
			if (replies >= 0) posts.addPostsCount(1 + replies);
			return posts;
		}
	}
	
	public static Threads createThreads(JSONArray jsonArray, ChanLocator locator) throws JSONException, ParseException
	{
		if (jsonArray != null && jsonArray.length() > 0)
		{
			Posts[] threads = new Posts[jsonArray.length()];
			for (int i = 0; i < jsonArray.length(); i++) threads[i] = createPosts(jsonArray.getJSONObject(i), locator);
