...
public class RoneryPostsParser implements GroupParser.Callback
{
	private final String mSource;
	private final RoneryChanLocator mLocator;
	...
	public RoneryPostsParser(String source, Object linked)
	{
		mSource = source;
	...
	public static Icon obtainIcon(RoneryChanLocator locator, String country, String countryName)
	{
		Uri uri = locator.buildPath("foolfuuka", "foolz", "foolfuuka-theme-foolfuuka", "assets-1.1.1",
				"images", "flags.png").buildUpon().appendQueryParameter("country", country).build();
		return new Icon(locator, uri, countryName);
	}
	...
	public boolean onStartElement(GroupParser parser, String tagName, String attrs) throws ParseException
	{
		...
		else if ("span".equals(tagName))
		{
			...
			else if (cssClass != null && cssClass.contains("flag"))
			{
				...
				if (matcher.find())
				{
					...
					mPost.setIcons(obtainIcon(mLocator, country, title));
				}
			}
		}
		...
		else if ("img".equals(tagName))
		{
			...
			if (cssClass != null && (cssClass.contains("thread_image") || cssClass.contains("post_image")))
			{
				...
				if (cssClass.contains("is_spoiler_image")) mAttachment.setSpoiler(true);
			}
		}
		return false;
	}
