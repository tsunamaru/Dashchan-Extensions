...
public class BritfagsChanLocator extends ChanLocator
{
	private static final Pattern BOARD_PATH = Pattern.compile("/[^/]+(?:/(?:\\d+\\.html)?)?");
	private static final Pattern THREAD_PATH = Pattern.compile("/[^/]+/res/(\\d+)\\.html");
	private static final Pattern ATTACHMENT_PATH = Pattern.compile("/[^/]+/src/\\d+\\.\\w+");

	public BritfagsChanLocator()
	{
		addChanHost("britfa.gs");
		addConvertableChanHost("www.britfa.gs");
	}
	...
	public String getBoardName(Uri uri)
	{
		...
		if (segments.size() > 0)
		{
			...
			if ("brian".equals(boardName) || "*".equals(boardName) || "sfw".equals(boardName) || "o".equals(boardName))
			{
				return null;
			}
			return boardName;
