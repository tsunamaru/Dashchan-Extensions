...
public class BunbunmaruPostsParser {
	private final String source;
	private final BunbunmaruChanConfiguration configuration;
	private final BunbunmaruChanLocator locator;
	...
	public BunbunmaruPostsParser(String source, Object linked, String boardName) {
		this.source = source;
		configuration = BunbunmaruChanConfiguration.get(linked);
		locator = BunbunmaruChanLocator.get(linked);
	...
	private static final TemplateParser<BunbunmaruPostsParser> PARSER = new TemplateParser<BunbunmaruPostsParser>()
		...
		if (holder.attachment != null) {
			if (holder.attachment.getFileUri(holder.locator) == null) {
		...
		if (holder.attachment.getSize() > 0 || holder.attachment.getWidth() > 0 || holder.attachment.getHeight() > 0) {
			holder.post.setAttachments(holder.attachment);
		}
		holder.attachment = null;
		return false;
	}).name("em").open((instance, holder, tagName, attributes) -> holder.attachment != null)
			.content((instance, holder, text) -> {
